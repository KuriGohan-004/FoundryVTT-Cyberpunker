...
Hooks.once("init", () => {
  ...
  // Setting for combat start playlist
...
// Show custom message, play sound, and play playlist on combat start
Hooks.on("combatStart", async (combat) => {
  // Show custom message
  ...
  // Play selected playlist once (and stop others)
  if (!combat.getFlag(MODULE_ID, "playlistPlayed")) {
    ...
    if (playlistId && game.user.isGM) {
      ...
      game.playlists.forEach(p => p.playing && p.stopAll());
